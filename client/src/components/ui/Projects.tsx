...
export function Projects() {
  ...
  const handleNext = () => {
    if (hasNextProjects) {
      setCurrentIndex(currentIndex + projectsPerView);
    }
  };
        ...
        <div className="relative">
          ...
          {projects.length > projectsPerView && (
            <div className="flex gap-4 mt-12 justify-end">
              <button
                onClick={handlePrev}
                disabled={!hasPrevProjects}
                data-testid="button-projects-prev"
                className="w-12 h-12 border border-white/20 rounded-full flex items-center justify-center text-white/60 hover:text-white hover:border-white/40 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
              >
                <ChevronLeft size={20} strokeWidth={1.5} />
              </button>
              <button
                onClick={handleNext}
                disabled={!hasNextProjects}
